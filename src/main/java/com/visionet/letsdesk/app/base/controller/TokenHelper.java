...
public class TokenHelper {
     ...
     * 保存token值的默认命名空间
     */
    public static final String TOKEN_NAMESPACE = "tzh.tokens";

    /**
     * 持有token名称的字段名
     */
    public static final String TOKEN_NAME_FIELD = "tzh.token.name";
    ...
    private static void setCacheToken(HttpServletRequest request, String tokenName, String token) {
        try {
            ...
            EhcacheUtil.set(tokenName0, token);
    ...
    public static boolean validToken(HttpServletRequest request) {
        ...
        String cacheToken = (String)EhcacheUtil.get(tokenCacheName);

        if (!token.equals(cacheToken)) {
            LOG.warn("validToken.internal.invalid.token Form token " + token + " does not match the session token " + cacheToken + ".");
            return false;
        }

        // remove the token so it won't be used again
        EhcacheUtil.remove(tokenCacheName);
