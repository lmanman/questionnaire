package com.visionet.letsdesk.app.common.modules.string;
	...
	public static void CheckIllegalCharacter(String content) throws RestException{
		for (int i = 0; i < content.length(); i++) {
			...
			if (Character.isHighSurrogate(ch) || Character.isLowSurrogate(ch)) {
				throw new RestException(MessageSourceHelper.GetMessages("app.common.utilities.string.StringUtil.illegal")+ch);
			}
		}
	}
...
//      y  = y * ( threehalfs - ( x2 * y * y ) );   // 2nd iteration, this can be removed
    ...
    public static void processAutoIncrement(String path,String out) throws Exception{
        String source = FileUtil.read(path);
        String key = "AUTO_INCREMENT=(.*?) ";
        Pattern pattern3 = Pattern.compile(key);
        Matcher matcher3 = pattern3.matcher(source);
        while (matcher3.find()) {
            String content = matcher3.group(1);
            String src = "AUTO_INCREMENT=" + content + " ";
            source = source.replace(src, "AUTO_INCREMENT=1 ");
        }
        FileUtil.write(new File(out),source);
    }
...
//            System.out.println(GetNextString("AA"));
            processAutoIncrement("/Users/xuetao/Downloads/dmp/wsk_ddl.sql","/Users/xuetao/Downloads/dmp/wsk_ddl3.sql");
        }catch (Exception e){
            e.printStackTrace();
        }
    }
