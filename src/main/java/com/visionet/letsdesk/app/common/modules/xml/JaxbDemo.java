...
import com.visionet.letsdesk.app.common.modules.mapper.JaxbMapper;
...
public class JaxbDemo {
	...
	public void objectToXml() {
		...
		user.getRoles().add(new Child(1L, "admin"));
		user.getRoles().add(new Child(2L, "user"));
	...
	public void xmlToObject() {
		...
		Parent user = JaxbMapper.fromXml(xml, Parent.class);
	...
	public void toXmlWithListAsRoot() {
		Parent user1 = new Parent();
		user1.setId(1L);
		user1.setName("calvin");

		Parent user2 = new Parent();
		user2.setId(2L);
		user2.setName("kate");

		List<Parent> userList = Lists.newArrayList(user1, user2);

		String xml = JaxbMapper.toXml(userList, "userList", Parent.class, "UTF-8");
